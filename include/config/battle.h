...
#define B_DARK_VOID_FAIL            GEN_1 // In Gen7+, only Darkrai can use Dark Void.
...
#define B_ILLUMINATE_EFFECT         GEN_LATEST // In Gen9+, Illuminate prevents accuracy reductions and ignores the target's evasion. I REMOVED THIS!
...
#define B_PROTEAN_LIBERO            GEN_1 // In Gen9+, Protean and Libero change the user's type only once per Battle.
#define B_INTREPID_SWORD            GEN_1 // In Gen9+, Intrepid Sword raises Attack by one stage only once per Battle.
#define B_DAUNTLESS_SHIELD          GEN_1 // In Gen9+, Dauntless Shield raises Defense by one stage only once per Battle.
...
#define B_FLAG_SLEEP_CLAUSE         FLAG_SLEEP_CLAUSE_TOGGLE     // If this flag is set, sleep clause is enabled; if the player / AI has already put a Pokémon on the opponent's side to sleep and it is still sleeping, another one can't be put to sleep. AI requires AI_FLAG_CHECK_BAD_MOVE to understand.
...
#define B_FAST_HP_DRAIN             VAR_HP_BAR_SPEED  // If set to TRUE, HP bars will move faster.
