...
AI_SINGLE_BATTLE_TEST("SWITCHING: AI_FLAG_SMART_SWITCHING: AI force switches out after main attacking stat drops to -3, if the mon on field loses the 1v1")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_SMART_SWITCHING | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_MON_CHOICES);
...
AI_SINGLE_BATTLE_TEST("SWITCHING: AI_FLAG_SMART_SWITCHING: AI will switch out if all moves deal zero damage")
{
    ...
    GIVEN {
        ...
        ASSUME(gMovesInfo[MOVE_POLTERGEIST].effect == EFFECT_POLTERGEIST);
...
AI_SINGLE_BATTLE_TEST("SWITCHING: AI_FLAG_SMART_MON_CHOICES: AI will consider choice-locked player priority when determining which mon to send out")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_DARMANITAN_GALAR_ZEN_MODE_TWO) { Speed(6); HP(1); Moves(MOVE_EARTHQUAKE); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("SWITCHING: EFFECT_EXPLOSION and EFFECT_SUPER_FANG should be ignored for bad odds calculations")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_SMART_SWITCHING | AI_FLAG_SMART_MON_CHOICES | AI_FLAG_OMNISCIENT);
        PLAYER(SPECIES_RATICATE){ Level(100); Moves(MOVE_SUPER_FANG, MOVE_TACKLE, MOVE_SPLASH, MOVE_EXPLOSION); }
        OPPONENT(SPECIES_BUTTERFREE) {Level(1); Moves(MOVE_TACKLE); }
        OPPONENT(SPECIES_ABOMASNOW){ Level(100); Moves(MOVE_WOOD_HAMMER); }
    } WHEN {
        TURN {
